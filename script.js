...
// =========================================
// NUMBER COUNTER ANIMATION (FAST FACTS)
// =========================================
document.addEventListener("DOMContentLoaded", function() {
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const animate = (timestamp) => {
                    ...
                    // Ease-out effect (starts fast, slows down smoothly at the end)
                    const easeOut = (t) => 1 - Math.pow(1 - t, 4);
                    const progressRatio = Math.min(progress / duration, 1);
                    const currentNum = Math.floor(easeOut(progressRatio) * target);
                    
                    counter.innerText = prefix + currentNum.toLocaleString() + suffix;
    ...
    counters.forEach(counter => {
        ...
        // 1. Ignore elements that don't contain numbers (like "Newly open")
        if (!/\d/.test(text)) {
            counter.classList.remove('counter');
            return;
        }
        
        // 2. Extract the number, prefix, and suffix cleanly
        const cleanText = text.replace(/,/g, '');
        const targetMatch = cleanText.match(/\d+/);
        if (!targetMatch) return;
        
        const targetNum = parseInt(targetMatch[0], 10);
        const parts = cleanText.split(targetMatch[0]);
        const prefix = parts[0] || '';
        const suffix = parts.slice(1).join(targetMatch[0]) || ''; // Restores symbols like "+" or ":1"
        
        // 3. Store properties for the animation
        counter.setAttribute('data-target', targetNum);
        counter.setAttribute('data-prefix', prefix);
        counter.setAttribute('data-suffix', suffix);
        
        // 4. Initialize at 0 before the animation starts
...
// =========================================
// GLOBAL DYNAMIC ZOOM BACKGROUND
// =========================================
document.addEventListener('DOMContentLoaded', () => {
    // 1. Inject the background element dynamically so we don't have to edit all HTML pages
    const bg = document.createElement('div');
    bg.className = 'global-bg';
    document.body.insertBefore(bg, document.body.firstChild);

    // 2. Scroll Zoom Logic using requestAnimationFrame for smooth performance
    let ticking = false;
    window.addEventListener('scroll', () => {
        if (!ticking) {
            window.requestAnimationFrame(() => {
                // Scale increases as you scroll down. (Change 0.0003 for speed)
                const scale = Math.max(1, 1 + (window.scrollY * 0.0003));
...
window.addEventListener('scroll', unflipCards, { passive: true });
window.addEventListener('touchmove', unflipCards, { passive: true });